...
namespace CommunityNetWork.Dal
{
    public static class Neo4JFormat
    {
       ...
       const string MatchDeleteLabelFormat = "(unlinkednode:{0}),(linkednode:{0})-[r]-()";
        ...
        public static string GetMatchNodeByPropertyString<TNode>(string property, object value)
          where TNode : INode
        {

            string label = TypeName<TNode>();
            string identifier = label.ToLower();
            string tag = value is string ? "'" : "";
            return string.Format("({0}:{1}{{{2}:{3}{4}{3}}})", identifier, label, property, tag, value, tag);
        }
